...
# Choisir des bases de mesure aléatoires pour Alice et Bob
#a_basis = 'X' if np.random.random() < 0.5 else 'Z'
#b_basis = 'X' if np.random.random() < 0.5 else 'Z'
a_basis = 'X'
b_basis = 'X'


# Simulons Eve interceptant le qubit avant que Bob ne mesure
eve_result = eve_interception(qc, 1)
print(f"Resultat de Eve: {eve_result}")

# Simuler les mesures sécurisées
alice_result, bob_result = measure_bell_state(qc, a_basis, b_basis)



# Alice et Bob communiquent leurs bases de mesure
print(f"Alice a mesuré dans la base {a_basis} et a obtenu {alice_result}")
print(f"Bob a mesuré dans la base {b_basis} et a obtenu {bob_result}")

# Vérifier les corrélations si les bases sont identiques
if a_basis == b_basis:
    print("Alice et Bob comparent leurs résultats de mesure.")
    correlated = alice_result == bob_result
    print("Les résultats sont-ils corrélés ? ", "Oui" if correlated else "Non")
else:
    print("Alice et Bob n'ont pas utilisé la même base, donc ils ne comparent pas leurs mesures.")
