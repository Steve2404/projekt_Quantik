...
class QuantumChatClient(tk.Tk):
    ...
    def repeat_process(self):
        """Répéter le processus jusqu'à ce que compter soit >= 3."""
        while self.compter < 3:
            # Réinitialiser les états avant chaque tentative
            self.O_basis = []
            self.O_check_bits = []
            self.O_index = []
            self.qber = 0.0
            ...
            if self.role.startswith('s'):
                self.display_message(">Envoyeur: Je suis dans repeat_processing: 1-debut")
                self.sender_init(self.get_n_bits())
                time.sleep(2)
                self.display_message(">Envoyeur: Je suis dans repeat_processing: 2-apres")
                self.sender_index(self.get_n_bits())
                self.continue_sender()
            elif self.role.startswith('r'):
                self.display_message(">receveur: Je suis dans repeat_processing: debut")
                self.receiver_qam(self.get_n_bits())

            if self.response:
                self.send_message_button.config(state='normal')
                messagebox.showinfo("Info", f"Vous devez encore refaire {3-self.compter} fois")
                self.compter += 1
                if self.compter >=3:
                    break
            else:
                messagebox.showinfo("Info", "Répétition du processus jusqu'à ce que la clé soit sécurisée.")
                self.compter = 0
    ...
    def receiver_qam(self, n_bits):
        """Préparer la base de Bob et mesurer le circuit quantique."""
        IBMProvider.save_account(token, overwrite=True)

        # Recevoir le circuit quantique de l'émetteur
        qc_str = base64.b64decode(self.QC).decode('utf-8')
        self.qc = QuantumCircuit.from_qasm_str(qc_str)

        # Générer la base de Bob
        self.basis, _ = self.generate_bb84_data(nb_bits=n_bits)
        self.send("BASIS", concatenate_data(self.basis))

        # Vérifier que le circuit quantique contient suffisamment de qubits
        if n_bits > self.qc.num_qubits:
            messagebox.showerror("Erreur", "Nombre de bits demandé supérieur au nombre de qubits disponibles dans le circuit.")
            return

        # Mesurer les qubits en fonction de la base de Bob
        try:
            self.qc = bob_measure(self.qc, self.basis)
            self.bits = calcul(self.qc)

            # Afficher les bits mesurés et le circuit
            self.display_message(f"{self.client_name} Bits: {self.bits}")
            self.display_message(self.qc.draw())
        except Exception as e:
            messagebox.showerror("Erreur de mesure", f"Erreur lors de la mesure des qubits : {e}")
