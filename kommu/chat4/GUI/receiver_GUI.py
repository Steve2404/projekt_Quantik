...
class QuantumChatClient(tk.Tk):
    ...
    def setup_ui(self):
        ...
        # Composants pour le choix du nombre de bits
        ...
        # Espace d'entrée pour les messages
    ...
    def process_server_message(self, action, content):
        ...
        if action == "ERROR":
            ...
            print(f"reception de Other check bits: {self.O_check_bits}")
            if self.role.startswith('s'):
    ...
    def sender_index(self, n_bits):
         # Réception des bases de l'autre client
        ...
        # Envoyer les bits de vérification
        self.choice_index, self.check_bits, self.key = checking(nb_bits=n_bits, alice_basis=self.basis, bob_basis=self.O_basis, bits=self.bits, bit_choice=self.choice_bit) 
    ...
    def continue_sender(self):
        ...
        print(f"self.check_bits: {self.check_bits}")
        print(f"self.O_check_bits: {self.O_check_bits}")
        print(f"self.choice_index: {self.choice_index}")
        print(f"self.choice_index: {self.choice_index}")
        ...
        self.display_message(f"Decision of {self.other_name}: {self.O_decision}")
        print(f"La decision est: {self.response}")
        if self.response:
    ...
    def receiver_qam(self, n_bits):
        ...
        # Préparer la réception des données
    ...
    def receiver_index(self):
        ...
        self.send("CHECK", concatenate_data(self.check_bits))
        print(f"receiver_index: {self.check_bits}")
